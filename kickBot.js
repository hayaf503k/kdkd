...
client.on('messageCreate', async (message) => {
  ...
  const row = new MessageActionRow()
    .addComponents(ment)
      ...
      embeds: [new MessageEmbed().setDescription(`** لتلفيل حسابك او لجعل حسابك اوتو رياكشن  أو لجعل حسابك ثابت فى الروم الصوتى من  فضلك قم باختيار الشىىء المناسب لك من المنيو**`).setThumbnail(message.guild.iconURL({dynamic : true})).setFooter({text : `نظام تلفيل حسابات برويوت` , value : `${message.guild.name}`}).setTitle('تلفيل بروبوت حسابات')],
      components: [row]
    });
  }
});
