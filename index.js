...
client.on('messageCreate', async (message) => {
  if (message.content.startsWith('-cre')) {
    ...
    const token = args[1]; // Extract the token from the command

    too.findOne({ guildId: message.guild.id }, async (err, data) => {
      if (!data) {
        too.create({
          guildId: message.guild.id,
          botTokens: [token], // Push the token into the botTokens array
        });
      } else {
        data.botTokens.push(token); // Push the token into existing botTokens array
        data.save();
      }
    });
